...
## 2026-05-18 Addendum: Current Architecture Baseline After Route Changes

This section is the current implementation baseline. Earlier learnings remain as historical context, but any conflict should be resolved in favor of this section and the active architecture/proposal docs.

### Active Product Path

- The default product renderer is the platform-neutral Rust layout engine, not VexFlow. VexFlow remains a lazy legacy comparison/fallback renderer and should not be imported by the default layout path.

- Browser startup initializes parser WASM only. Layout WASM must stay lazy and load only when the layout renderer is actually invoked.

- The active browser packages are split by runtime responsibility:
  - `src/wasm/parser-pkg-web/` for browser parser initialization
  - `src/wasm/layout-pkg-web/` for browser layout scene generation
  - `src/wasm/parser-pkg-node/` for Node parser initialization when needed
  - `src/wasm/layout-pkg-node/` for CLI/Node layout rendering

- The old combined `src/wasm/pkg/` / `src/wasm/drummark_wasm.ts` path is not the active production contract. Older notes that say the app imports the checked-in package under `src/wasm/pkg` are superseded.

### Parser Ownership

- Rust/WASM parser output is the production parser path for app, worker, docs build, and CLI normalization. Lezer/regex-parser notes are historical unless a current file still explicitly uses them for tests or migration checks.

- Parser-facing TypeScript code should go through the explicit parser runtime registry and wrappers, not browser-only WASM modules. `src/wasm/skeleton.ts` is package-neutral and calls the registered parser implementation.

- When parser behavior disagrees with old Lezer or regex behavior, the deciding authority is the spec plus current Rust/WASM parser contract, not legacy parser parity.

### Rendering Boundary

- Layout rendering is source-to-scene through `build_layout_scene(source, options)` in the layout WASM package. The TypeScript SVG adapter is intentionally thin: it renders `LayoutScene` primitives and composites and should fail loudly on unknown scene item kinds.

- `setLayoutSource` and module-level source caches are obsolete. Render calls must carry the source attached to the accepted parsed-score revision so rapid edits cannot render a stale source against a newer score.

- Front-end preview pagination must consume page-aware layout APIs (`renderScorePagesToSvgs` / `renderScenePagesToSvgs`) because `LayoutScene.pages` may contain multiple pages.

- CLI SVG rendering initializes parser WASM through the Node parser wrapper and layout WASM through the Node layout wrapper. CLI rendering should fail closed on internal layout/adapter errors rather than writing friendly placeholder SVGs.

### Build and Verification

- `npm run wasm:build` is the authoritative local rebuild command. It builds parser/layout packages for browser and Node, verifies generated declaration boundaries, and reports raw/gzip/brotli WASM sizes.

- `npm run build` runs the WASM build first and then builds TypeScript/docs/Vite artifacts. Manual `wasm-pack` instructions in older entries are historical troubleshooting notes, not the preferred workflow.

- `npm run verify:split-wasm` is the current full gate for the split architecture. It covers build, targeted tests, import-boundary enforcement, parser/layout semantic parity, browser network audit, and representative CLI SVG generation.

- Static import boundaries are part of the contract:
...
### Superseded Learning Buckets

- VexFlow-specific implementation notes remain useful only when touching the legacy renderer or comparing visual parity. They should not guide new default-renderer work unless the task explicitly targets VexFlow.

- Lezer migration, regex-parser, and parser-coexistence notes are historical. They explain why earlier decisions changed, but new parser features should target the Rust/WASM parser path.

- Single-package WASM rebuild notes involving `src/wasm/pkg`, `drummark_wasm.ts`, or hand-run `wasm-pack` commands are superseded by the split-package build script unless the task is explicitly debugging old history.

- Old notes describing `renderScoreToSvg()` as single-page-only are superseded for preview integration. Single-page helpers still exist for focused adapter tests, but app preview must use page arrays.

- Older renderer parity assertions based on VexFlow DOM classes (`vf-*`) are stale for the layout engine. Stable layout-engine assertions should use scene/SVG semantic attributes such as `data-role`, `data-measure-id`, and page/system structure.
